...
function conversorMercado(mercado) {
  const linha_mercado = nomes_mercados
    .filter((linha) => linha.includes(mercado))
    .flatMap((linha) => linha);

  if (!linha_mercado.length) {
    return { banco: mercado, tela: mercado };
  }

  return {
    banco: linha_mercado[0],
    tela: linha_mercado[linha_mercado.length - 1],
  };
}

function conversorProduto(produto) {
  const linha_produto = nomes_produtos
    .filter((linha) => linha.includes(produto))
    .flatMap((linha) => linha);

  if (!linha_produto.length) {
    return { banco: produto, tela: produto };
  }

  return {
    banco: linha_produto[0],
    tela: linha_produto[linha_produto.length - 1],
  };
}
